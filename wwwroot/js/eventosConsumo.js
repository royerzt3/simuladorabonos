...
function cotizadorPrincipal() {
    ...
    $("#idCompPaises").change(function () {
        $('#idCompStatusCam').val = "-1";
        _strIdPais = $('#idCompPaises option:selected').attr('value');
        if (_strIdPais == "0") {
            limpiarCombo("idCompCanales");
            var select = document.getElementById("idCompCanales");
            ...
            callServiceCanalesxPais(_strIdPais);
            
        }
      
        //callSelectTipoClte(parseInt(_strIdPais));
    ...
    $("#idBuscarCamp").click(function () {

        var strPais = parseInt( $('#idCompPaises').val());
        var strCanales = parseInt($('#idCompCanales').val());
        var strStatus = parseInt($('#idCompStatusCam').val());
        if (parseInt(strPais) <= 1) {
            mostrarModalMensaje(1, "Es necesario seleccionar un pais");
        } else if (parseInt(strCanales) < 1) {
            mostrarModalMensaje(1, "Es necesario seleccionar un canal");
        } else {
            var jsonPeticion = { fiPais: "", fiCanal:""};
            if (strStatus != -1) {
                jsonPeticion.fiStatus = strStatus;
            }
            jsonPeticion.fiPais = strPais;
            jsonPeticion.fiCanal = strCanales;
            callServiceCampa(jsonPeticion);
        }
        $("#checktodos").removeAttr("checked");
    ...
    $("#idBtnAgregarSkus").click(function () {
    ...
    $('#carga02').change(function (e) {
        ...
        let urlSucursales =urlSimulador + "sucursales";
        ...
        console.log("CanalId:. " + strIdCanal + " IdTipoTienda:. " + strTipoTienda)
...
function generaTablaSku(LstSkus) {
    ...
    $.each(LstSkus, function (key, value) {
        ...
        dataTable += '<tr class="valorTblSku">' +
            ...
            '<td id="tdSku' + i + '">' + value.fcdeslarga2 + '</td>' +
            '<td id="tdDesc' + i + '" style="text-align:left;">' + value.camp_id + '-' + value.fcdescripcion + '</td>' +
            '<td id="tdPrecioD' + i + '">' + value.fddescripcion + '</td>' +           
